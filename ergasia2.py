def Bad_Words(fin):#i create a function and for parameters i put the file
    bad_letters = fin.read().replace(' ','\n').replace('(','').replace(')','').replace(',', '\n').splitlines()#assign the file into bad_letters (the name of the list) and i put the replace method in some symbols
    Bad_Letters=["f","c","k","r","F","C","K","R"]#i create another list with my bad consonants
...
while True: #check the elements of the file with a loop
  if not char1:#if it don't find the first character
    print "Your file is empty, please write something!" #that means the file is empty
...
#http://ebooks.edu.gr/modules/document/file.php/DSEPAL-C219/%CE%94%CE%B9%CE%B4%CE%B1%CE%BA%CF%84%CE%B9%CE%BA%CF%8C%20%CE%A0%CE%B1%CE%BA%CE%AD%CF%84%CE%BF/%CE%A3%CE%B7%CE%BC%CE%B5%CE%B9%CF%8E%CF%83%CE%B5%CE%B9%CF%82%20%CE%9C%CE%B1%CE%B8%CE%B7%CF%84%CE%AE/24-0601-01_Programmatismos-Ypologiston_G-EPAL_Simeioseis-Mathiti.pdf <--eleghos orthothtas, eisagwgh ston programmatismo, vivlio epal,page 54
