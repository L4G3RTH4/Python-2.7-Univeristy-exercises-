def largest_word(fin):     #create a function
    ...
    for i in range(0,5):    #create a loop from the first element
     try:   #its like the if attribute
        word = split_sentence[i].replace('a','').replace('e','').replace('i','').replace('o','').replace('u','').replace('y','')
        .replace('A','').replace('E','').replace('I','').replace('O','').replace('U','').replace('Y','')
        #We used the .replace() attribute to replace the vowels with null
...
#inside the file replaces the line change into space and the comma, dot and parenthesis into null 
